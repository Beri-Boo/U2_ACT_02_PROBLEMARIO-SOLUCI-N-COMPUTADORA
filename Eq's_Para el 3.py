...
kf = float(input("Introduce la concentración de esfuerzo por fatiga a la flexión (Kf): "))
kfs = float(input("Introduce la concentración de esfuerzo por fatiga a la torsión (Kfs): "))

print("\nDatos de Carga")
Ma = float(input("Introduce el Momento Alternante (Ma): "))
Mm = float(input("Introduce el Introduce el Momento Medio (Mm): "))
Ta = float(input("Introduce el Torque Alternante (Ta): "))
Tm = float(input("Introduce el Torque Medio (Tm): "))

print("\nDatos Extras Para las Cuatro Formulas")
Syt = float(input("Resistencia a la Fluentica (Syt): "))
Se = float(input("Límite de Resistencia a la Fatiga (Se): "))
...
#Grafica de q
X_q = np.array([0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]) #r,mm (columna)
Curvas_q = np.array([60, 100, 150, 200]) #las 4 curvas (filas)
y_curvasq = np.array([
    #r:0.5, 1, 1.5, 2, 2.5, 3.0, 3.5, 4.0
    [0.56, 0.65, 0.70, 0.73, 0.75, 0.77, 0.78, 0.79, 0.79, 0.79], # Curva Sut = 60 kpsi
    [0.68, 0.76, 0.80, 0.83, 0.85, 0.87, 0.88, 0.89, 0.89, 0.89], # Curva Sut = 100 kpsi
    [0.76, 0.84, 0.88, 0.90, 0.92, 0.93, 0.94, 0.94, 0.94, 0.94], # Curva Sut = 150 kpsi
    [0.83, 0.90, 0.93, 0.95, 0.96, 0.97, 0.98, 0.98, 0.98, 0.98]  # Curva Sut = 200 kpsi
])
...
X_qc = np.array([0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]) #r,mm (columna)
Curvas_qc = np.array([60, 100, 150, 200]) #las 4 curvas (filas)
y_curvasqc = np.array([
    #r:0.5, 1, 1.5, 2, 2.5, 3.0, 3.5, 4.0
    [0.64, 0.73, 0.78, 0.82, 0.84, 0.86, 0.87, 0.88, 0.88, 0.88], # Curva Sut = 60 kpsi
    [0.74, 0.82, 0.86, 0.89, 0.91, 0.92, 0.93, 0.94, 0.94, 0.94], # Curva Sut = 100 kpsi
    [0.82, 0.88, 0.91, 0.93, 0.94, 0.95, 0.96, 0.97, 0.97, 0.97], # Curva Sut = 150 kpsi
    [0.88, 0.92, 0.94, 0.95, 0.96, 0.97, 0.98, 0.98, 0.98, 0.98]  # Curva Sut = 200 kpsi
])
...
#ahora la grafica q
xl_q = np.linspace(0.5, 5, 100) #si flota el punto, es porque r es <0.5mm
...
#limpieza visual
#plt.tight_layout, automatico, agusta los margenes para que no se encimen,   plt.show, comando de ejecutión, detiene el código y espera para que se vean
plt.tight_layout()
plt.show()
